...
func (s *OverviewService) Build(now time.Time) *domain.Overview {
	...
	for _, c := range commands {
		...
		if c.SentAt.After(now.Add(-24 * time.Hour)) {
			ov.Commands.Today++
		}
	}
	...
	for i := 0; i < len(commands) && i < 5; i++ {
		recentCmd = append(recentCmd, *commands[i])
	}
	ov.RecentCommands = recentCmd
	recentAudits := s.store.ListAudits(5)
	ov.RecentAudits = make([]domain.AuditLog, 0, len(recentAudits))
	for _, a := range recentAudits {
		ov.RecentAudits = append(ov.RecentAudits, *a)
	}
